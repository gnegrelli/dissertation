...
By varying voltage frequency on the rotor circuit, this type of generator is able to supply power to the grid in a wider range of wind speed, reaching $\pm 30\%$ of rated slip. In addition, the converters provide independent control of both real and reactive power, thus ending the necessity of reactive compensators \cite{Muljadi2010}. Since approximately $30\%$ of rated power flows through the rotor windings, power electronics components have lower specifications and do not have great impact on overall costs. On the other hand, these generators need regular maintenance due to slip rings, brushes and gearbox, preventing its broad use in offshore applications \cite{Yaramasu2015}.
...
\section{Wind Turbine Generator Model Selected for Parameter Estimation}
...
In this model, the WPP is represented by its Thevenin equivalent, composed of voltage source and impedance, connected to the grid. The direct and quadrature components of the equivalent voltage source are controlled by a series of blocks that simulate the WTGs controllers. The block diagram of this model is shown in Figure \ref{fig: ErlMod},
...
\noindent where $v_{T}$ and $\phi_{v}$ correspond to the voltage magnitude and angle at substation bus, $P$ and $Q$ are the active and reactive power generated by the WPP, $v_{d}$ and $v_{q}$ stand for the generated voltage's direct and quadrature components, $R$ and $X$ correspond to the stator equivalent resistance and reactance, $k_{I}$ and $T_{I}$ are the PI block gain and time constant, $T_{V}$ is the delay block time constant and $k_{VC}$ is the voltage block gain.

Terminal voltage $v_{T}$ is the variable of interest in this model, appearing as input to DFIG control system. Active and reactive current are calculated using that input, according to equation \eqref{eq: Currents}.
...
\begin{equation}
	\begin{cases}
		P = \frac{R(v_{Td}v_{d} + v_{Tq}v_{q} - v_{T}^{2}) + X(v_{Tq}v_{d} - v_{Td}v_{q})}{R^{2} + X^{2}} \\
		Q = \frac{X(v_{Td}v_{d} + v_{Tq}v_{q} - v_{T}^{2}) - R(v_{Tq}v_{d} - v_{Td}v_{q})}{R^{2} + X^{2}}
	\end{cases}
	\label{eq: Outputs}
\end{equation}
