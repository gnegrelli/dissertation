...
Mutation step takes place right after gene selection. At first, each selected gene receives a random value $x'_{i}$ between [0,1] that will be used as an input to a mapping function based on the mean and variance of each particular gene on the population. The mapping function is describe by equation \eqref{eq: mappingf}.
...
The process goes on until a stop criterion has been fulfilled.
